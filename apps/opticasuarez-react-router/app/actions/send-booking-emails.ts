...
export async function sendBookingEmails(bookingDetails: BookingDetails) {
  try {
    const selectedLocation =
      locations[bookingDetails.location as keyof typeof locations];
...
Óptica Suárez
    ...
    console.log(
      "Optica email payload:",
      JSON.stringify(opticaEmailPayload, null, 2)
    );

    const opticaEmailResult = await resend.emails.send(opticaEmailPayload);
    console.log(
      "Optica email result:",
      JSON.stringify(opticaEmailResult, null, 2)
    );

    // Send confirmation email to customer
    console.log(
      `Attempting to send confirmation email to customer: ${bookingDetails.email}...`
    );
    ...
    console.log(
      ...
      JSON.stringify(customerEmailPayload, null, 2)
    );

    const customerEmailResult = await resend.emails.send(customerEmailPayload);
    console.log(
      "Customer email result:",
      JSON.stringify(customerEmailResult, null, 2)
    );

    console.log(
      `Emails sent successfully to optica@lorenzogm.com and ${bookingDetails.email}`
    );
