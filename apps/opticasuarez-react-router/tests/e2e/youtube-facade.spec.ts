...
test.describe("YouTube Facade Performance", () => {
  test("should display YouTube thumbnail initially without loading iframe", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await expect(
      page.getByText("Tu óptica de confianza en Jaén")
    ).toBeVisible();
    ...
    const playButtonContainer = page.locator(
