...
test.describe("Homepage Content Verification", () => {
  test("should display correct hero section content", async ({ page }) => {
    ...
    await expect(
      page.getByText(homepageContent.hero.description)
    ).toBeVisible();
  });

  test("should display correct video about section content", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await expect(page.locator("h1")).toContainText(
      homepageContent.videoAbout.title
    );
    await expect(
      page.getByText(homepageContent.videoAbout.description)
    ).toBeVisible();
  });
  ...
  test("should display correct services grid content", async ({ page }) => {
    ...
    for (const service of homepageContent.servicesGrid.items) {
      ...
      await expect(
        page.locator("article h3").filter({ hasText: service.title })
      ).toBeVisible();
      ...
      await expect(
        page
          ...
          .locator("p")
          .filter({ hasText: service.description })
      ).toBeVisible();
    }
  });
  ...
  test("should display correct social media information", async ({ page }) => {
    ...
    await expect(
      page.getByText(homepageContent.socialMedia.instagram.title)
    ).toBeVisible();
    await expect(
      page.getByText(homepageContent.socialMedia.facebook.title)
    ).toBeVisible();

    // Check for Instagram section with handle
    const socialMediaSection = page
      .locator("section")
      .filter({ hasText: "Estamos en Instagram" });
    await expect(socialMediaSection).toBeVisible();

    // Check for Facebook section
    const facebookSection = page
      .locator("section")
      .filter({ hasText: "Estamos en Facebook" });
  ...
  test("should display specialists section content", async ({ page }) => {
    ...
    await expect(
      page.getByText(homepageContent.specialists.title)
    ).toBeVisible();
    await expect(
      page.getByText(homepageContent.specialists.subtitle)
    ).toBeVisible();
    await expect(
      page.getByText(homepageContent.specialists.description)
    ).toBeVisible();
  });
  ...
  test("should display locations information", async ({ page }) => {
    ...
    for (const location of homepageContent.locations.locations) {
      ...
      const phoneElement = page
        .getByText(location.phone)
        .or(page.getByText(location.phone.replace(/-/g, "")))
        .or(page.getByText(location.phone.replace(/-/g, " ")));
  ...
  test("should display navigation elements", async ({ page }) => {
    ...
    await expect(
      page.locator("nav").getByRole("link", { name: "Inicio" })
    ).toBeVisible();
    await expect(
      page.locator("nav").getByRole("link", { name: "Quienes Somos" })
    ).toBeVisible();
    await expect(
      page.locator("nav").getByRole("link", { name: "Servicios" })
    ).toBeVisible();
    await expect(
      page.locator("nav").getByRole("link", { name: "Blog" })
    ).toBeVisible();
    await expect(
  ...
  test("should display WhatsApp booking functionality", async ({ page }) => {
    ...
    await expect(
      page.getByText(bookAppointmentSection.description)
    ).toBeVisible();
  ...
  test("should display news section", async ({ page }) => {
    ...
    if (homepageContent.news) {
      ...
      await expect(
        page.getByText(homepageContent.news.buttonText)
      ).toBeVisible();
    }
  });
});
